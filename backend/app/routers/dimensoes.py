...
class AlunoIn(BaseModel):
    ...
    sexo: Optional[str] = None             # 'M' | 'F' | 'N'
    nome_pai: Optional[str] = None
    nome_mae: Optional[str] = None


# Qualquer usuário autenticado pode cadastrar/listar aluno
@router.post("/alunos")
def criar_aluno(body: AlunoIn, user=Depends(require_roles())):
...
@router.get("/alunos")
def listar_alunos(user=Depends(require_roles())):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
            SELECT id_aluno, nome, data_nascimento, sexo, nome_pai, nome_mae
            FROM aluno_dim
            ORDER BY id_aluno
            """
        )
...
def criar_professor(body: ProfessorIn, user=Depends(require_roles())):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
...
def criar_materia(body: MateriaIn, user=Depends(require_roles())):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO materia_dim (nome) VALUES (%s) RETURNING id_materia, nome",
            (body.nome,),
        )
        return cur.fetchone()
...
def criar_periodo(body: PeriodoIn, user=Depends(require_roles())):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
...
def criar_tipo_avaliativo(body: TipoAvaliativoIn, user=Depends(require_roles())):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
...
@router.post("/datas")
def criar_data(body: DataCalendarioIn, user=Depends(require_roles())):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
